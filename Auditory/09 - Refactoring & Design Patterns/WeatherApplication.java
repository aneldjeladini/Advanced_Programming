...
class WeatherDispatcher{
    Set<WeatherSubscriber> weatherSubscribers = new HashSet<>();

    public void setMeasurements(float temperature, float humidity, float pressure){
        weatherSubscribers.forEach(ws -> ws.update(temperature,humidity,pressure));
    }

    public void register(WeatherSubscriber weatherSubscriber){
        weatherSubscribers.add(weatherSubscriber);
    }

    public void remove(WeatherSubscriber weatherSubscriber){
        weatherSubscribers.remove(weatherSubscriber);
    }
}
